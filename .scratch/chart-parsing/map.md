...
- The user crops to a **rectangle**; Cells inside it that are background are marked **Non-stitch** and render transparent. ~~The rule is flood-fill from the rectangle's outer edge~~ — the corpus shows at least two mechanisms in the wild (background silhouette, and X glyphs over ordinary Cells), and choosing between them is now [Decide how Non-stitch Cells are identified](issues/07-non-stitch-rule.md).
...
- **The correction vocabulary.** Now that every parse is reviewed, what the user can actually *do* is first-class and unspecified: repaint a Cell, repaint a Run, merge two Palette entries, nudge the grid, mark Cells Non-stitch, re-crop, start over. Which of these v1 needs depends on which errors [Automatic extraction spike](issues/05-extraction-spike.md) actually produces — inventing the list before seeing them would be designing for imagined failures. Graduates once the spike reports. Likely several tickets.
- **Whether the app surfaces its own uncertainty.** No longer load-bearing — nothing routes on it now that the human is the acceptance test — but two silent-failure modes are known (skew-induced wrong pitch, over-segmented Palette) and a reviewer eyeballing 17,000 Cells will miss both. Whether confidence highlighting is worth building depends on whether the spike yields a usable signal for free.
- **Deployment shape of the parsing service.** Hosting, cold-start tolerance, request size limits, and how a mobile client uploads a multi-megabyte screenshot. Waits on knowing what the pipeline actually costs to run.
- **Whether the symbol layer ever becomes semantic.** Increase/decrease symbols are noise for v1, but a knitting app that eventually reads shaping instructions would want them. Out of v1, in scope for the project — revisit once [Overlay layer](issues/06-overlay-layer.md) has measured them.
