...
- [Chart JSON contract](issues/08-chart-contract.md) — **the parser↔client seam, settled concretely.** `cells: int[][]` of Palette indices (image orientation, `-1` = Non-stitch, never a Palette entry); Palette entries `{rgb, name}` with **array index as identity**; two never-averaged confidence signals (whole-Chart `chart` scalar + sparse per-Cell review list); a `source` block (crop/pitch/origin/skew) so correction can re-overlay; single-integer `schema_version`. Reading direction stays a readout toggle, never stored. **Completes the map's spine** — nothing else to decide before the parser has a buildable output shape. Hands [09](issues/09-correction-vocabulary.md) the fields its correction tools need.
- [The correction vocabulary](issues/09-correction-vocabulary.md) — **the five candidate capabilities collapse to three primitives + accelerator + attention signal + escape hatch.** Cell-tier: (1) **repaint a selection** (Cell or Run-span) to any Palette entry or `-1` — folds in mark-Non-stitch, since `-1` is just a paintable value; (2) **flood-fill-from-tap** inherited from [07](issues/07-non-stitch-rule.md) as the silhouette accelerator; (3) **review the flagged Cells**, consuming 08's per-Cell review list so "every parse is reviewed" isn't a blind scan. Structural-tier (redo, Cell edits discarded by design): (4) **Palette entry add/remove/recolour** — merge/split fall out of this + repaint, no dedicated tool; (5) **adjust `source` and re-parse** (one op for re-crop *and* grid-nudge, snap-to-lattice inside it); (6) **start over**, the zero case of (5). Repaint/Re-parse kept **out** of `CONTEXT.md` (operation verbs, not artifact nouns). **Last open ticket — the frontier is now empty.**
...
- **On-device Chart storage shape.** IndexedDB schema and eviction for persisted Charts. [Chart JSON contract](issues/08-chart-contract.md) gives the client its full storage payload plus `schema_version`; how the client stores and evicts it is a build decision, symmetric with client-framework and readout-UI above. Was in-scope fog awaiting the contract; the contract resolved it out.
